...
keep.peaks <- which(as.character(seqnames(granges(subset_atac))) == main.chroms)
...
motif_analysis<-function(k){
  ...
  print(i)
  write.csv(differential.activity,paste(motif_path,"/motif_analysis_cluster_",k,".csv", sep=""),quote = F)
  
  if(nrow(differential.activity)==0){
  print("no motif found")
  }else{
  png(file=paste(motif_path,"/motif_analysis_cluster_",k,".png", sep=""), width = 8, height = 4, unit="in", res=200)    
  print(MotifPlot(
   object = subset_atac,
